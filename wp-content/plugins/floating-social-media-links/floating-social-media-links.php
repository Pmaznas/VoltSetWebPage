...
function fsml_perform_update( $version_from = '1.1.0', $input ) {
	// additions in most recent version
...
?>
	<div class="wrap">
		...
		/* CSS3 Transitions */
			...
			#jstest { color: #f00; font-weight: bold; }
			...
			.fsml_testimg {opacity: .6; filter:alpha(opacity=60); <?php if(fsml_897_getOption('borderradius') != 'none') { echo 'border-radius: 8px; '; }?>width: 50px; } .fsml_testimg:hover {opacity: 1; filter:alpha(opacity=100);}
			.upload_image_button:hover { cursor: pointer; }
			::selection { background-color: #fff000; color: #000; text-shadow: none; } /*get rid of the ugly default blue, just for fun, and wordpress' text shadow (really???)*/
			a:hover { cursor: pointer; }
			ul#themes-list { list-style-type: none; margin: 0; padding: 0; }
			#themes-list li { width: 16%; padding: 0; margin: 0 .3%; float: left; max-width: 180px; }
			#themes-list img { max-height: 150px; cursor: pointer; margin-bottom: 7px; }
			.selected-theme { outline: 5px solid rgb(184, 216, 18); box-shadow: 6px 6px 4px #000; }
			#themes-list img:hover { outline: 4px solid #fff000; }
		</style>
		...
		<?php if(!wp_style_is('wp-color-picker', 'registered')){ ?><h4 style="color: #f00;">WARNING: you are using an out-of-date version of WordPress that this plugin no longer supports. You will not be able to use the color pickers or upload media directly, but if there is also a JavaScript error on this page, then you will only be able to do very minimal things with this plugin until you upgrade to WordPress 3.5 or higher. If you have recently updated this plugin, everything should still work on the front end of your site, but your administration capabilities will be quite limited.</h4> <?php } ?>
		<ul id="fsmlsubpages">
		...
		<form method="post" action="options.php" id="fsmlopform">
			...
			if(!array_key_exists('plugin_version',$options)||$options['plugin_version'] != '1.5.0') {
			...
			<table class="form-table" id="fsml_general">
				...
				<!-- IE7 Fix -->
				<tr valign="top" style="border-top:#ddd 1px solid;" class="genadvanced">
				...
				<tr valign="top" class="genadvanced" id="metaviewport" <?php if(!fsml_897_getOption('hidefromsmall')){ echo 'style="display:none;"'; } ?>>
				...
				<!-- Cookie Duration -->
				<tr valign="top" class="genadvanced" id="cookielength" <?php if(!fsml_897_getOption('usecookies')){ echo 'style="display:none;"'; } ?>>
			...
			<!-- the content subpage -->
			<tr><td colspan="2"><p><small>If you need more links, link reordering, or want to preview coming features, you may want to try the <a href="http://celloexpressions.com/dev/floating-social-media-links/fsml-contents-widgets-temporary-plugin-information/" target="_blank">FSML Contents with Widgets TEMPORARY BETA Plugin</a>.</small></p></td></tr>
				<!-- Facebook Page URL -->
				<tr>
				...
				<tr id="fsmlcustomlink1" <?php if($options['numactivecustom'] < 1) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage1']; ?>" class="fsml_testimg" id="customurlimage1_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink2" <?php if($options['numactivecustom'] < 2) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage2']; ?>" class="fsml_testimg"  id="customurlimage2_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink3" <?php if($options['numactivecustom'] < 3) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage3']; ?>" class="fsml_testimg"  id="customurlimage3_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink4" <?php if($options['numactivecustom'] < 4) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage4']; ?>" class="fsml_testimg"  id="customurlimage4_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink5" <?php if($options['numactivecustom'] < 5) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage5']; ?>" class="fsml_testimg"  id="customurlimage5_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink6" <?php if($options['numactivecustom'] < 6) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage6']; ?>" class="fsml_testimg"  id="customurlimage6_preview"/></a>
					</td>
				</tr>
				...
				<tr id="fsmlcustomlink7" <?php if($options['numactivecustom'] < 7) { echo 'style="display:none;"'; } ?>>
					...
					<td>
						...
						<img src="<?php echo $options['customurlimage7']; ?>" class="fsml_testimg"  id="customurlimage7_preview"/></a>
					</td>
				</tr>
			</table>
			...
			<table class="form-table" id="fsml_buttons">
				<tr><td colspan="2"><p><small>If you need pintrest, google+, linkedin, or flattr action buttons, link and button reordering, or want to preview coming features, you may want to try the <a href="http://celloexpressions.com/dev/floating-social-media-links/fsml-contents-widgets-temporary-plugin-information/" target="_blank">FSML Contents with Widgets TEMPORARY BETA Plugin</a>.</small></p></td></tr>
				...
				<tr valign="top" id="fburltolike" <?php if(!fsml_897_getOption('fblike')) { echo 'style="display: none;"'; } ?>>
				...
				<tr><td colspan="2"><em>Note: as of Floating Social Media Links version 1.5, no action buttons have significant impacts on page load time. The facebook like button uses a custom implementation and the actual button is only loaded if/when a user clicks the like button, keeping with the models of the follow and subscribe buttons.</em></td></tr>
			</table>
			...
			<table class="form-table" id="fsml_appearance">
				...
				<tr valign="top" style="border-top: 1px solid #999;" class="apadvanced">
			...
			<table class="form-table" id="fsml_design" >
				...
				<ul id="themes-list">
					<li><h4>Bright</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-bright.png',__FILE__); ?>" id="theme-bright" onclick="setTheme('bright')" <?php if($theme == 'bright') echo 'class="selected-theme"'; ?> /><br/>
					...
					<li><h4>Dark</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-dark.png',__FILE__); ?>" id="theme-dark" onclick="setTheme('dark')" <?php if($theme == 'dark') echo 'class="selected-theme"'; ?> /><br/>
					...
					<li><h4>Elegant</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-elegant.png',__FILE__); ?>" id="theme-elegant" onclick="setTheme('elegant')" <?php if($theme == 'elegant') echo 'class="selected-theme"'; ?> /><br/>
					...
					<li><h4>Subtle</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-subtle.png',__FILE__); ?>" id="theme-subtle" onclick="setTheme('subtle')" <?php if($theme == 'subtle') echo 'class="selected-theme"'; ?> /><br/>
					...
					<li><h4>Contrast</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-contrast.png',__FILE__); ?>" id="theme-contrast" onclick="setTheme('contrast')" <?php if($theme == 'contrast') echo 'class="selected-theme"'; ?> /><br/>
					...
					<li><h4>Customize...</h4>
						<img src="<?php echo plugins_url('/img/admin/theme-custom.png',__FILE__); ?>" id="theme-customize" onclick="setTheme('custom')" <?php if($theme == 'custom') echo 'class="selected-theme"'; ?> /><br/>
				...
				<tr class="dadvanced" >
					...
					<!-- Border Color -->
					<tr valign="top" id="bordercolor" <?php if($options['fixeddynamic'] == 'dynamic' || $options['border'] == 'no') { echo 'style="display: none;"'; } ?>>
				...
				<!-- Frame Hover Effect -->
				<tr valign="top" style="border-top: 1px solid #ddd;" class="dadvanced">
				...
				<!-- Border Radius -->
				<tr valign="top" class="dadvanced" style="border-top: 1px solid #ddd;">
				...
				<!-- Show Border? -->
				<tr valign="top" id="showborder" <?php if($options['fixeddynamic'] == 'dynamic'){ echo 'style="display: none;"'; } ?> class="dadvanced">
...
//these need to go in the head so that the overriding options can happen in the body (footer)
	echo '<link rel="stylesheet" id="fsmlStyleSheet-1-5-0" href="' . plugins_url('fsml-base.css?ver=1.5.0', __FILE__) . '" type="text/css" media="all" />';
...
function fsml_output($phplink1 = null, $phplink2 = null, $phplink3 = null){
	//set the outputs to vars, to avoid weird bugs with the Wordpress Settings API
		...
		echo ('<style type="text/css">/*disclaimer: this css is php-generated, so while it isnt pretty here it does look fine where its generated*/'
			...
			if ($frameshadowyn != 'no') { echo ' box-shadow: 0 0 2px ' . $frameshadowcolor . '; '; }
			else { echo 'box-shadow: none;'; }
			if ( fsml_897_getOption('border') != 'yes' || fsml_897_getOption('fixeddynamic') == 'dynamic' ) { echo 'border: none;'; }
		echo '}';
		if(fsml_897_getOption('leftright') == 'right'){
			echo ' #fsml_ff, #fsml_ffhidden { right: 0;  top: ' . $fsml_897_top . $fsml_897_top_un . ';}';
			echo ' #fsml_fblikemodal { left: -185px; }';
			...
			echo ' #fsml_ff, #fsml_ffhidden { left: 0;  top: ' . $fsml_897_top . $fsml_897_top_un . ';}';
			echo ' #fsml_fblikemodal { right: -185px; }';
			echo ' #fsml_ff { border-radius: ' . $brsize . '; }';
			if(fsml_897_getOption('closeoption') != 'no'){
		...
		if(fsml_897_getOption('framehovereffect') == 'yes'){
			echo '#fsml_ff, 
			#fsml_ffhidden {
				opacity: 0.5;
			}
		...
		if(fsml_897_getOption('grayscaleicons')){
		...
		if(fsml_897_getOption('fblike') ){
		   ...
		   <?php $fblikecode = '<img src="'.plugins_url('/img/like.png',__FILE__).'" id="fsml_fblike" onclick="fsml_openfblike()" />';
			$fbmodalcode = '<div id="fsml_fblikemodal" style="display:none;">
				<iframe id="fsml_fblikeframe" frameborder="0" ></iframe><hr/>
		...
		<div id="fsml_ff" <?php if (fsml_897_getOption('closeoption') == 'starthidden') { echo 'style="display:none;"'; } if (fsml_897_getOption('customzindex')){ echo 'style="z-index: '.fsml_897_getOption('customzindex').';"'; } ?>>
			<?php if (fsml_897_getOption('closeoption') != 'no') { ?> <img src="<?php echo ($fsmlbaseurl_897 . $fsml_897_xurl); ?>" class="fsml_xlr fsmlopenclose" id="fsml_hide" title="hide" onclick="fsmlHide(<?php echo "'$hsan','$hsuc','$hscl'"; ?>)" style="z-index: <?php if (fsml_897_getOption('customzindex')){ echo 1 + fsml_897_getOption('customzindex'); } else{ echo 10000; } ?>;" /> <?php } ?>
		...
		<?php if (fsml_897_getOption('closeoption') == 'yes' || fsml_897_getOption('closeoption') == 'starthidden') { ?>
			<div id="fsml_ffhidden"  class="fsml_showhide" <?php if (fsml_897_getOption('closeoption') == 'starthidden' ) { echo 'style="display:block;"'; } else { echo 'style="display:none"'; }  if (fsml_897_getOption('customzindex')){ echo 'style="z-index: '.fsml_897_getOption('customzindex').';"'; } ?>>
				<img src="<?php echo $fsmlbaseurl_897 . $fsml_897_plusurl; ?>" alt="expand" onclick="fsmlShow('<?php echo $hsan; ?>')" title="<?php echo $expandtitle; ?>" class="fsmlopenclose"/>
			</div>
		<?php }
} //fsml_output
?>